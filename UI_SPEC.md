...
개발 PC의 설치 안내에만 「메인 화면」 버튼. 루프백이거나 공개 호스트에서 이 개발 PC로 판별된 경우에만 보인다. 공개 사이트의 다른 방문객에는 없다. `?devpc=1`은 화면 선택만 한다. 루프백은 설치형 3열(로컬 API). 공개 호스트는 배포 웹 3열(배포 백엔드). localhost로 보내지 않는다. 웹 화면에는 폴더·시작 프로그램·프로그램 종료가 없다.
